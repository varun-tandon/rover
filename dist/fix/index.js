 ...
 */
async function fixIssueInBatch(issue, worktreePath, verbose, onProgress) {
 ...
 */
export async function runBatchFix(options, onProgress) {
    ...
    for (const [i, issue] of issues.entries()) {
        ...
        const result = await fixIssueInBatch(issue, worktreePath, verbose, (msg) => onProgress({
            phase: 'fixing',
            currentIssueId: issue.id,
            currentIssueIndex: i + 1,
            totalIssues: issues.length,
    ...
    for (let iteration = 1; iteration <= maxIterations; iteration++) {
        ...
        if (iteration < maxIterations) {
            ...
            const iterationPrompt = buildIterationPrompt('batch', analysis.items.filter(i => i.severity === 'must_fix' || i.severity === 'should_fix'));
            try {
                await runClaude({
