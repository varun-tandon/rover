 ...
 */
function parseTextContent(line) {
    try {
 ...
 */
function containsMarker(output, marker) {
 ...
 */
export function isAlreadyFixed(output) {
 ...
 */
export function isReviewNotApplicable(output) {
 ...
 */
export function extractDismissalJustification(output) {
    const lines = output.split('\n');
    let fullText = '';
    for (const line of lines) {
        if (!line.trim())
 ...
 */
export function buildInitialFixPrompt(issueId, issueContent) {
 ...
 */
export function buildIterationPrompt(issueId, actionableItems) {
