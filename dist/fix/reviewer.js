 ...
 */
async function getDefaultBranch(worktreePath) {
 ...
 */
async function getGitDiff(worktreePath) {
 ...
 */
function parseReviewToolUsage(line) {
    try {
 ...
 */
function parseReviewTextContent(line) {
    try {
        const parsed = JSON.parse(line);
 ...
 */
function extractReviewText(output) {
 ...
 * The reviewer only has read-only access to the codebase (Read, Glob, Grep, LS)
 */
export async function runReview(worktreePath, options = {}) {
 ...
 */
export async function runBugReview(worktreePath, options = {}) {
 ...
 */
export async function runPerformanceReview(worktreePath, options = {}) {
 ...
 */
export function hasActionableItems(analysis) {
    if (analysis.isClean) {
        return false;
    }
    // Only "must_fix" and "should_fix" items require iteration
    // "suggestion" items don't block completion
    return analysis.items.some((item) => item.severity === 'must_fix' || item.severity === 'should_fix');
}
/**
