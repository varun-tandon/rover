 ...
 */
function formatIssueForBatch(issue: CandidateIssue, index: number): string {
 ...
 */
export async function runChecker(options: CheckerOptions): Promise<CheckerResult> {
  ...
  for (const batch of batches) {
    ...
    try {
      ...
      const batchResult = parseBatchResponse(
        result.resultText ?? '',
        batch.map((i) => i.id)
      );
