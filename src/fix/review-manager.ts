...
async function getCommitSummary(
  ...
  const result = await execGit(
    ['log', '--oneline', `${defaultBranch}..HEAD`],
    worktreePath
  );
 ...
 */
function generatePRTitle(issueId: string, issueSummary?: string): string {
...
function generatePRBody(
  ...
  if (issueContent) {
    ...
    for (const line of lines) {
      ...
      if (!inDescription && trimmed.startsWith('---')) continue;
  ...
  sections.push('');
  sections.push('---');
  sections.push('Generated by [Rover](https://github.com/anthropics/rover)');
 ...
 */
export async function listFixes(targetPath: string): Promise<ReviewListResult> {
  ...
  const validFixes = state.fixes.filter((fix) => {
    if (fix.status === 'merged') return true; // Keep merged for history
 ...
 */
export async function getFixDetails(
  targetPath: string,
  issueId: string
): Promise<{
  fix: FixRecord | null;
 ...
 */
export async function cleanupFix(
  targetPath: string,
  issueId: string
): Promise<{ success: boolean; error?: string }> {
  const resolvedPath = resolve(targetPath);
  ...
  if (worktreeExists(fix.worktreePath)) {
    const result = await execGit(
      ['worktree', 'remove', '--force', fix.worktreePath],
      resolvedPath
    );
 ...
 */
export async function cleanupAllFixes(
  targetPath: string,
  options?: {
