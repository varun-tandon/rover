 ...
 */
export function getRoverignorePath(targetPath: string): string {
 ...
 */
export async function loadRoverignore(targetPath: string): Promise<Ignore> {
 ...
 */
export async function filterDiffByRoverignore(
  targetPath: string,
  diff: string
): Promise<string> {
  const ig = await loadRoverignore(targetPath);
  ...
  const filteredSections = sections.filter(section => {
    ...
    if (!match?.[2]) return true; // Keep sections we can't parse
